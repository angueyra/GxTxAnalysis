classdef iAnalysisGUI < genericGUI
   properties
       iA
   end
   
   methods
       function hGUI=iAnalysisGUI(iA,params,fign)
          if nargin == 0
            fign=10;
          end
       ...
       function tagButtonCall(hGUI,hObject,~)
           ...
           currWavei=hGUI.iA.HEKAnamefind(currWave);
           
           hGUI.iA.tags{currWavei}=tag;
           fprintf('tagged %s as %s\n',hGUI.iA.waveNames{currWavei},tag);
           set(hGUI.figData.infoTable,'Data',Selected)
           hGUI.nextButtonCall();
           hGUI.updatePlots();
           hGUI.enableGui;
       ...
       function RowNames=waveTableNames(hGUI)
           Rows=size(hGUI.iA.waveNames,1);
           colors=pmkmp(Rows,'CubicL');
           tcolors=round(colors./1.2.*255);
           RowNames=cell(size(Rows));
           for i=1:Rows
               RowNames{i}=sprintf('<html><font color=rgb(%d,%d,%d)>%s</font></html>',tcolors(i,1),tcolors(i,2),tcolors(i,3),hGUI.iA.waveNames{i});
           end
       end
   end
